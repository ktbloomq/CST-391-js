...
export const readAlbums: RequestHandler = async (req: Request, res: Response) => {
    try {
        ...
        res.status(500).json({
            message: 'There was an error fetching the albums'
        });
    }
};
    ...
    try {
        ...
        req.body.tracks.forEach(async (track: Track, index: number) => {
            try {
                ...
                res.status(500).json({
                    message: 'There was an error writing album tracks'
                });
            }
        });
...
async function readTracks(albums:Album[], res: Response<any, Record<string, any>>) {
    for(let i=0; i<albums.length; i++) {
        try {
            ...
            res.status(500).json({
                message: 'There was an error ferching album tracks'
            });
        }
    }
}
...
export const deleteAlbum: RequestHandler = async (req:Request, res:Response) => {
    try {
        ...
        res.status(500).json({
            message: 'There was an error deleting albums'
        });
    }
}
